...
def projects():
    posts = BlogPost.query.all()
    return render_template("projects.html", all_posts=posts, logged_in=current_user.is_authenticated)

@app.route("/pubs")
def pubs():
    return render_template("pubs.html", logged_in=current_user.is_authenticated)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        hash_with_salt = generate_password_hash(request.form["password"], method='pbkdf2:sha256', salt_length=8)
        new_user = User(
            name = request.form["name"],
            email = request.form["email"],
            password = hash_with_salt
        )
...
def login():
    ...
    if request.method == "POST":
        ...
        # Find user object by email entered
        user = User.query.filter_by(email=email).first()
        # Email doesn't exist
        if not user:
            flash("That email does not exist, please try again.")
            return redirect(url_for('login'))
        # Check stored procedure hash against entered password hashed
        elif not check_password_hash(user.password, password):
            flash("Password incorrect, please try again.")
            return redirect(url_for('login'))
        else:
            login_user(user)
            return redirect(url_for('aboutme', user_id=user.id))
